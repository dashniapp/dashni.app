...
export default function ChatScreen({ route, navigation }) {
    ...
    return (
      <View style={[styles.msgRow, isMe && styles.msgRowMe]}>
        ...
        <View style={{ maxWidth: '72%' }}>
          ...
          >
            {isFreeMale && !isMe ? (
              <View style={styles.lockedMsgContent}>
                <Text style={[styles.bubbleText, { color: 'transparent', overflow: 'hidden' }]}>
                  {item.content || '• • • • • •'}
                </Text>
                <View style={styles.lockedMsgOverlay}>
                  <Ionicons name="lock-closed" size={12} color="#888" />
                  <Text style={styles.lockedMsgOverlayText}>Upgrade to read</Text>
                </View>
              </View>
  ...
  // Free male with no messages yet — don't render the conversation UI at all
  if (isFreeMale && setupDone && messages.length === 0) {
    return (
      <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <View style={styles.freeMaleLockWrap}>
          <Ionicons name="lock-closed" size={48} color="rgba(255,255,255,0.25)" />
          <Text style={styles.freeMaleLockTitle}>Upgrade to Premium to message</Text>
          <TouchableOpacity style={styles.lockedBarBtn} onPress={() => navigation.navigate('Paywall')}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}>
        ...
        {isFreeMale ? (
          <TouchableOpacity style={styles.lockedBar} onPress={() => navigation.navigate('Paywall')} activeOpacity={0.85}>
            ...
            <Text style={styles.lockedBarText}>Upgrade to Premium to reply</Text>
        ...
        <View style={[styles.inputBar, (!hasAccess || isFreeMale) && { display: 'none' }]}>
...
const styles = StyleSheet.create({
  ...
  freeMaleLockWrap: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 16, paddingHorizontal: 32 },
  freeMaleLockTitle: { color: colors.textSecondary, fontSize: 16, fontWeight: '500', textAlign: 'center' },
  lockedMsgContent: { position: 'relative' },
  lockedMsgOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4 },
  lockedMsgOverlayText: { color: '#888', fontSize: 11 },
});
